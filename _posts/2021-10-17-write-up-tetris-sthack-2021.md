...
Lorsque l'on arrive dans le challenge, on a deux modes possible :
...
À noter aussi, que la vitesse des blocs est accéléré plus les niveaux avances.
...
Mais les équipes participantes ont trouvées deux solutions différentes de résoudre ce challenge. 
...
La première étape est de savoir comment communiquer avec le jeu pour pouvoir execute des actions dans le jeu.
On sait déjà Unity WebGL est fait en partie en Web assembler. Si on connait un peu la technologie web assembler on sait qu'il y a une communication entre le Javascript et le code compilés.  
...
Attention, seule les méthodes publiques sont utilisable !
...
Le projet WABT est une suite d'outil développé par le projet Webassembly et qui permet en autre de facilité le debbug des binaire Webassembly. 
Le projet est disponible sur github [wabt](https://github.com/WebAssembly/wabt "Le github du projet WABT").

Dans cette suite d'outil, c'est wasm2wat qui nous être utile. En effet cet outil permet de transcrire le webassembly dans un langage human readable.
{% highlight bash %}
...
Pour récupérer les noms des GameObject et de leurs méthode, on faut explorer les métadata du jeu. 
Pour extraire le fichier de métadata dans AssetStudio, il faut tout d'abord récupérer les data du jeu. 
Pour cela, il faut regarder dans l'onglet "Réseau" de la console du navigateur est récupérer le fichier TetrisChallenge.data.
...
Puis, il faut sélectionner le fichier .dat puis sélectionner le dossier de reception
...
Et en cherchant bien, on retrouve le nom des GameObject suivit de leurs méthodes et des variables.
...
Il faut donc qu'il y ait un paramètre à la fonction pour être utilisé et puis dans le fichier .wat ou dans le global-data, on voit des méthodes qui parle d'index, cela signifie que les blocs ont des index et sont donc appelable, grâce à ça.
On essaye donc :
{% highlight js %}
...
![spawner_problem](/assets/images/sthack_2021/spawner_problem.png)

Il faut donc essaye autrement. On peut essayer de passer par le Next et faire spawner les prochains blocs comme on le souhaite et le bloc déjà le mettre de le hold.
{% highlight js %}
unity_instance.SendMessage('Next', 'spawnNext', 1)
{% endhighlight %}

Ca fonctionne, mais si l'on l'execute une seconde fois on se rend que le next fait spawner un autre type de bloc, cela veut dire que la méthode spawnerNext ne prend pas en compte le paramètre que l'on passe.
...
## Coder le script
L'idée du script est de faire apparaitre le bloc de la barre vertical (qui est l'index 6 après avoir testé les différents index) et de résoudre 4 lignes par 4 lignes histoire d'avoir le maximum de score à la fois. 
...
Cette solution a été utilisé par la team "ZenSec" et présenté dans leur write-up disponible [ici](https://zensecctf.github.io/sthack2021/2021-10-19-sthack2021-tetris "Write up craking"), donc n'hésitez pas à jeter un oeil.

# Solution 3 - Utiliser l'extension Cetus
C'est la solution qui a été utilisé par la team "Les Pire hat" et présenté dans leur write-up disponible [ici](https://lespireshat.medium.com/sthack-2021-write-up-tetris-ddf27faf6a35 "Write up cetus"), donc n'hésitez pas à jeter un oeil.